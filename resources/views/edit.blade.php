...
<input type="text" name="title" value="{{$post->title}}" class="h-18 bg-white border border-gray-300
...
inline-block mb-6 shadow-lg rounded hover:shadow">Cancel</a>

<form action="/posts/{{$post->id}}" method="get">
@csrf
@method('DELETE')
<button class="ml-4 bg-red-500 tracking-wide text-white px-6 py-2
